...
### Client Corrections & New FAQ Page — /o-nas, formularz kontaktowy, /faq, SEO (2026-09-25)

Three sets of changes combined into one spec (`context/features/client-corrections-spec.md`,
committed with the feature): copy corrections on `/o-nas` and the contact modal, extracting the
`/o-nas` closing CTA into Sanity, and a brand-new `/faq` page with 5 categories / 50 CMS-managed
Q&A pairs, plus two AI-visibility SEO items from a separate client document. Branch
`feature/client-corrections`, cut from `main`.

- **The spec's technical assumptions needed reconciling before any code was written — user
  confirmed the approach up front.** "Kontakt" being described as a "podstrona" in the spec was
  just the client not knowing the technicalities: there is no `/kontakt` route and never has been —
  it's the `ContactFormDialog` modal opened from `Navbar`. Item 2's corrections landed on
  `ContactFormDialog.tsx`/`ContactForm.tsx` directly, no page was built. Similarly, the spec's
  "make sure AboutHero's body matches the homepage" note was a wording-consistency remark, not a
  request to merge `aboutSection` (home teaser) and `aboutPage.storyBody` (`/o-nas` story) —
  two fields deliberately split apart on 2026-06-16. Left untouched, per the user's explicit call.
  This "smooth over/ignore what the client got technically wrong" reading is now a standing rule
  for this project's feedback specs (saved to memory).
- **`/o-nas` corrections:** `AboutHero`'s hardcoded eyebrow "Complex sp. z o.o." → "CCOMPLEX SP. Z
  O.O."; `aboutPage.heroSubheadline` reworded to the client's requested, less formal copy.
- **The "Masz pytania?" CTA extracted into Sanity** — new `aboutPage.cta` object (7 fields: eyebrow,
  headline, description, both button labels/hrefs), new "CTA" field group. `AboutCta.tsx` now reads
  from it with in-component Polish fallbacks (the repo's standard `initialValue`-doesn't-backfill
  pattern). **Kept the existing two buttons exactly as they were** (`/#kontakt`, `/wycena`) rather
  than the spec's suggested phone-call button — the client's actual ask was "let me edit this
  text," not "change what it does."
- **Contact modal corrections, all landed on the real hardcoded strings** (no CMS document exists
  for this modal): `Dialog.Description` gained "do nas" and a comma ("odpowiemy najszybciej, jak to
  możliwe"); the RODO consent label gained a visible `*`; the footnote below the submit button
  gained the same missing comma. Zod validation for the RODO consent was **already** required
  (`refine val === true`, since Round 4/5) — this was a visual-only fix.
- **New `/faq` page.** `faqPage` fixed-id singleton (`HelpCircleIcon`) + reusable `faqCategory`/
  `faqItem` objects, registered in `structure/index.ts` ("Strona FAQ", after "Strona Realizacje")
  and `sanity.config.ts` (`/faq` `mainDocuments` + `locations`). `FaqHero`/`FaqAccordion`/`FaqCta`
  (`app/components/faq/`) modeled on `OfferIndexGrid`'s header pattern and `OfferBrands`'s accordion
  pattern instead of the spec's literal Tailwind classes, for visual consistency with the rest of
  the site — one `Accordion.Root` per category so opening a question in one category doesn't affect
  another. Nav entries added to `Navbar` (right after "Realizacje") and `Footer`'s "Firma" column;
  `/faq` added to `sitemap.ts` via a new `faq` field on `sitemapQuery`.
- **Seeded + published via direct Sanity mutate API calls** (no MCP available this session) —
  dry-run first (`dryRun=true`), same precedent as Round 7. Audited `aboutPage` for a pending draft
  first (none), patched the published doc directly; `faqPage` didn't exist yet, created fresh via
  `createOrReplace`. Verified via GROQ readback: 5 categories, 50 Q&A items, all 7 `cta` fields.
- ⚠️ **A real bug was caught during verification, not shipped.** The new `parsePolishAddress` util
  (backing the `LocalBusiness` JSON-LD upgrade below) initially mis-parsed the **live**
  `footer.contactAddress` value — "Kępska 12, 45-130 Opole, **pok.20 (parter)**" — because the
  original implementation rejoined every segment after the first comma before matching the postal
  code, so the room-number suffix leaked into `addressLocality` as "Opole,pok.20 (parter)". Fixed
  to only use the first two comma-separated segments; added a regression test with the exact live
  value. Caught by checking the real production data before trusting the parser, not by reasoning
  about the code in isolation.
- **`OrganizationJsonLd.tsx` upgraded** (it already existed from the 2026-08-21 audit quick-wins
  feature): `@type` is now `["Organization", "LocalBusiness"]`, `address` is a structured
  `PostalAddress` when it parses (plain-string fallback otherwise), plus a hardcoded `areaServed`
  (the two voivodeships already named throughout the site's own copy — not CMS-managed, same
  reasoning as other hardcoded business facts).
- **`robots.ts` got an explicit `OAI-SearchBot` rule** alongside the existing wildcard `allow: '/'`
  — redundant (the wildcard already covers it) but directly satisfies the client's SEO document at
  no real cost.
- **Studio NOT redeployed this session** — the new "Strona FAQ" entry and `aboutPage`'s "CTA" field
  group aren't visible in the hosted Studio editor yet, though all seeded content is already live on
  the site (same pattern as every prior schema-adding feature; needs `npm run deploy` from
  `studio/`).
- **Commit hygiene:** staged only the 29 files this feature actually touched, explicitly excluding
  the ~216-file pre-existing CRLF/prettier drift already sitting in the working tree (verified via
  `git diff --stat` that only a handful of files carried real content changes) — same precedent as
  every prior feature's "left untouched" note. Also caught and fixed a mistake mid-session: the
  first commit included a `Co-Authored-By: Claude` line despite `CLAUDE.md`'s explicit "do not add
  Claude to any commit messages" — amended before merging.
- **6 new tests** for `parsePolishAddress` (standard address, multi-word city, no comma, malformed/
  missing postal code, empty string, and the live-data three-segment case). Nothing else was
  unit-testable — the rest of this feature is schema, content and presentational components.
- Verified: **184/184 Vitest** (178 baseline + 6 new), `type-check` (both workspaces, 0 errors),
  ...
  renders exactly **5** category headings and **50** accordion triggers; `/o-nas` shows the
  corrected eyebrow/subheadline and the CMS-driven CTA; the contact modal shows the corrected
  description, a visible `*` on the RODO consent, and the corrected footnote comma; `Navbar`/
  `Footer` both link to `/faq`; `/robots.txt` lists the `OAI-SearchBot` rule; `/sitemap.xml`
  includes `/faq`; the homepage's JSON-LD `<script>` shows `["Organization","LocalBusiness"]`, a
  correctly-parsed structured `PostalAddress`, and `areaServed`. 3 console errors, all
  environmental (Vercel Speed Insights 404 + Sanity Live CORS on the ad-hoc port), matching the
  documented precedent from prior rounds.
- **Left untouched (same precedent as prior features):** the pre-existing uncommitted `.mcp.json`,
  `AUDIT.md`, `OPTIMIZATION-PLAN.md`, ~28 unrelated spec markdown files, `OfferIndexGrid.tsx`,
  `package-lock.json` (all pure CRLF/prettier drift this feature never touched) — plus the untracked
...
- **One finding scoped down after checking the actual schema**, not left as a generic "add
  ...
  this was explicitly _not_ reported as a finding (operating rule: don't recommend something that
...
- **The "Tarasy accordion" turned out to be the shared `OfferBrands`/`brands` mechanism** used on
  _every_ offer page, not something Tarasy-specific — the client had repurposed the generic
...
_"ostatecznie - nie robimy zakładek… - poradze sobie"_. Not built.
...
- **#6 Offer-gallery rework.** `bentoClass()`'s 2×2 hero cell deleted; flat
  ...
  **GSAP ordering bug caught before shipping:** the footer/CTA block was initially lumped into the
  ...
  _before_ the grid cells despite sitting visually below them — split into its own
...
- ⚠️ **No Sanity MCP and no write token this session** — a first for this repo's history of
  ...
  Elewacje _draft_ copy and had nothing to do with what the live site actually serves).
...
- **Not driven in-browser:** a real contact-form _send_ (would email the dev inbox through Resend)
